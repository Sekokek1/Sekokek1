    ...
    static void Main(string[] args)
    {
        ...
        string pacmanSvg = $@"<svg width='800' height='200' xmlns='http://www.w3.org/2000/svg'>
            <style>
                ...
                @keyframes move {{ 0% {{ transform: translateX(30px); }} 100% {{ transform: translateX(770px); }} }}
                @keyframes chomp {{ 0% {{ transform: scaleY(0.1); }} 100% {{ transform: scaleY(1); }} }}
            ...
            <text x='200' y='112' class='date-text item1' text-anchor='middle'>{DateTime.UtcNow.ToString("dddd", CultureInfo.InvariantCulture).ToUpper()}</text>
